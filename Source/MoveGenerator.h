 ...
 * @date   April 2016
 * @brief  This file contains structure definitions and members of the MoveGenerator class.
 */
...
/**
 * @brief The MoveGenerator class is a simple AI class that generates moves for an associated Player instance.
 */
...
public:
    ...
    /**
   * @brief MoveGenerator is the default constructor for objects of the MoveGenerator class.
  ...
  explicit MoveGenerator(QObject* parent = 0);
    ...
    /**
   * @brief ~MoveGenerator is the default destructor for objects of type MoveGenerator
   */
  virtual ~MoveGenerator();

    /**
   * @brief aiPlayer is an accessor method to the AI Player associated with this MoveGenerator instance.
   * @return the currently associated AI player for this MoveGenerator instance.
   */
  QSharedPointer<Player> aiPlayer() const;

  /**
   * @brief setAiPlayer is a mutator method that associates a Player instance with this MoveGenerator instance.
  ...
  /**
   * @brief associatedGameBoard is an accessor method to the game Board associated with this MoveGenerator instance.
   * @return the currently associated Board as a pointer.
   */
  Board* associatedGameBoard() const;

  /**
   * @brief associateGameBoard is a mutator method that associates a game Board with this MoveGenerator instance.
...
public slots:
  ...
  /**
   * @brief handleTurnChange is the function that gets invoked when it is the AI's turn to move
  ...
  /**
   * @brief handleMoveCompletionRequired is called after a move has been initiated, in order to complete the move
   */
  void handleMoveCompletionRequired();

private:
  /**
   * @brief _aiPlayer is a shared pointer type that stores an instance of the AI player created by Chess
   */
  QSharedPointer<Player> _aiPlayer;

  /**
   * @brief _theGameBoard is a raw pointer to the game Board as created by the Ui::Chess class.
   * It cannot be a shared or scoped pointer, as the developer has no control over its owner,
   * and creating shared or scoped pointers of the Board will result in multiple deletion,
   * and utlimately an application crash on exit.
   */
  Board* _theGameBoard;

  /**
   * @brief _locationStart the location where the AI player will begin its next move.
   */
  boardCoordinateType _locationStart;

  /**
   * @brief _locationEnd the location where the AI player will move to from _locationStart.
   */
  boardCoordinateType _locationEnd;

  /**
   * @brief _containerForMoving contains only valid moves at the time the AI player needs to move, as it can then just grab a move from the container.
   */
  boardCoordinatesType _containerForMoving;

  /**
   * @brief _moveStrategy just an integer used in a simple round-robin method of determining how to make the next move.
   */
  static int _moveStrategy;

  /**
   * @brief _reverseIterateThroughPieces is a boolean that indicates whether to iterate through moves from the back or front of the containers.
   * It randomizes piece and move selection somewhat.
   */
  static bool _reverseIterateThroughPieces;

  /**
   * @brief moveTheFirstPieceThatCanMove does as its name suggests.  In some cases, priority is given to moves that can capture enemy pieces.
  ...
  /**
   * @brief moveWithPriorityForAttack wrapper to call moveTheFirstPieceThatCanMove with.
